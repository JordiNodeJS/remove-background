...
export const removeBackgroundLinkController: RequestHandler = async (
  ...
  // Inyectar la función de procesamiento real en la request
  (req as any)._processImage = async (
    res: Response,
    done: (processingTime: number) => void
  ) => {
  ...
  // Encolar la petición
  imageProcessingQueue.enqueue(req, res);
};